...
public class UserController {
  ...
  @PostMapping("/signup")
  public ResponseEntity<CommonResult> signup(@RequestBody UserInfo userInfo) throws Exception {
	User users = new User();
	userInfo.setConfirmed("0");
	userInfo.setUsertype("user");
    BeanUtilsCopy.copyPropertiesNoNull(userInfo, users);
    ...
    try {
	    // send email
	    mailService.sendHTMLMail(userInfo.getEmail(), userInfo.getUsername());
    }catch (Exception e){
    	e.printStackTrace();
	...
	/*
	 * @GetMapping("/confirmed/{username}") public String
	 * activeUserByUsername(@PathVariable("username") String username) throws
	 * Exception { // user click this link from email received to confirmed user
	 * if(userService.setConfirmedByUsername(username, "1")>0) { // return
	 * ResponseEntity.ok().body(CommonResult.build(Const.COMMONRESULT_OK_CODE,
	 * "User have confirmed!")); System.out.println("发送html文本文件-发生成功");
	 * logger.error("User have confirmed!"); return "<a href='"+ loginlink
	 * +"'>please click here to login SMC system</a>"; } return
	 * "User confirm action failed!"; // return
	 * ResponseEntity.ok().body(CommonResult.build(Const.COMMONRESULT_ERROR_CODE,
	 * "User confirm action failed!")); // return ResponseEntity.ok().body(new
	 * ResponseBean(OK.value(), OK.getReasonPhrase()).data("User have confirmed!"));
	 * }
	 */
  
  @PostMapping("/settings")
  public ResponseEntity<CommonResult> updateUserInfo(@RequestBody UserInfo userInfo) throws Exception {
	  
	  String username = userInfo.getUsername();
	  String oldpw = userInfo.getPassword();
	  String newpw = userInfo.getNewpassword();
	  ...
	  // validate old pw
	  User oneuser = userService.getUserByUsernameAndPassword(username, oldpw);
	  if (oneuser == null) {
	...
	/*
	 * @GetMapping("/logout/{username}") public ResponseEntity<CommonResult>
	 * logout(@PathVariable("username") String username) throws Exception {
	 * 
	 * // login, changepw, logout will update updatets column
	 * if(userService.setUpdatetsByUsername(username, new Date())>0) return
	 * ResponseEntity.ok().body(CommonResult.build(Const.COMMONRESULT_OK_CODE,
	 ...
	 * 
	 * @ResponseStatus(UNAUTHORIZED) public ResponseEntity<ResponseBean>
	 * handleAuthentication401Exception(AuthenticationException exception) throws
	 * Exception { return ResponseEntity.status(UNAUTHORIZED) .body(new
	 * ResponseBean(UNAUTHORIZED.value(),
	 * UNAUTHORIZED.getReasonPhrase()).error(exception.getMessage())); }
	 * 
	 * @ExceptionHandler(AuthenticationException.class)
	 * 
	 * @ResponseStatus(FORBIDDEN) public ResponseEntity<ResponseBean>
	 * handleAuthentication403Exception(AuthenticationException exception) throws
	 * Exception { return ResponseEntity.status(FORBIDDEN).body(new
	 * ResponseBean(FORBIDDEN.value(),
	 * FORBIDDEN.getReasonPhrase()).error(exception.getMessage())); }
	 */
